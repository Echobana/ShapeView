...
CanvasConsole::CanvasConsole(unsigned long long length, unsigned long long height)
        : _length(length)
        , _height(height)
{
...
void CanvasConsole::line(point_t start, point_t finish)
{
    if (start.x > finish.x)
...
void CanvasConsole::draw_pixel(point_t position) {
    _pixels[position.x + position.y * _length].color = '*';
}

Canvas *CanvasConsole::create(unsigned length, unsigned height) {
    return new CanvasConsole(length, height);
}